...
#include "dpdk_replication.h"

#include <vppinfra/error.h>
#include <vppinfra/elog.h>

/* include a local symbolic link pointing to the flowtble plugin headers */
#include "flowtable.h"
#include "flowdata.h"
    ...
    vlib_node_runtime_t * node, vlib_frame_t * frame)
{
    ...
    while (n_left_from > 0)
    {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2)
        {
            ...
            {
                c1 = vlib_dpdk_clone_buffer(vm, b0);
                dup_frame->n_vectors++;
    ...
    if (n_left_to_next > 0)
    {
        ...
        if (vlib_buffer_alloc(vm, &ebi, 1) == 1)
        {
            ...
            void * lib = dlopen("flowtable_plugin.so", RTLD_LAZY);
            ASSERT(lib);
            flowtable_main_t * ftm = dlsym(lib, "flowtable_main");
            ASSERT(ftm);
